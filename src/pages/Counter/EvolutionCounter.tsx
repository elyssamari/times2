...
export default function EvolutionCounter() {
  // Lazy initialization of the state
  const [count, setCount] = useState(() => {
    const savedCount = localStorage.getItem('evolutionCount');
    return savedCount !== null ? parseInt(savedCount, 10) : 0;
  });

  // Update localStorage whenever count changes
  useEffect(() => {
    localStorage.setItem('evolutionCount', count.toString());
  }, [count]);
    ...
    <Grid container gap={3} flexDirection='column'
      ...
      <Grid item>
        <Typography variant='h5' fontFamily='Inconsolata, monospace' fontWeight='bold' color='white'>
          Times Elyssa said 'Evolve-tion'</Typography>
      </Grid>
      <Grid item>
        <Grid container direction='row' justifyContent='center' gap={3} alignItems='center'>
          ...
          <Box borderRadius='10px' bgcolor='#435585' width='100px' height='100px' alignContent='center'>
